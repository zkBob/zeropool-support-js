...
class DirectBackend extends backend_1.ZeroPoolBackend {
    constructor(web3, snarkParams, config, state, worker) {
        super(state, worker);
        this.web3 = web3;
    ...
    async signAndSendPrivateTx(privateKey, privateTx) {
        const address = this.web3.eth.accounts.privateKeyToAccount(privateKey).address;
        const tx = await private_tx_1.EthPrivateTransaction.fromData(privateTx.data, privateTx.txType, this.zpState.privateAccount, this.snarkParams, this.web3, this.worker);
