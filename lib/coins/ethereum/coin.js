...
class EthereumCoin extends coin_1.Coin {
    constructor(mnemonic, web3, config, backend, worker) {
    ...
    cachePrivateTx(raw) {
        ...
        if (hashes.length < utils_2.CONSTANTS.OUT + 1) {
            const zeroes = Array(utils_2.CONSTANTS.OUT + 1 - hashes.length).fill('0');
            hashes.push(...zeroes);
        }
