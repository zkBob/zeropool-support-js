...
const minter_abi_json_1 = __importDefault(require("../evm/abi/minter-abi.json"));
...
class TronClient extends client_1.Client {
    constructor(rpc, privateKey, config) {
        ...
        this.tokenContracts = new Map(); // tokenAddress -> contract object
        this.poolContracts = new Map(); // poolAddress -> contract object
        this.ddContracts = new Map(); // ddQueueAddress -> contract object
        this.minterContracts = new Map(); // minterAddress -> contract object
        ...
        this.supportedMethods = new Map(); // contractAddress+method => isSupport
        this.rpcUrl = rpc;
    ...
    async getTokenContract(tokenAddress) {
        ...
        if (!contract) {
            contract = await this.tronWeb.contract(usdt_abi_json_1.default, tokenAddress);
            if (contract) {
    ...
    async getPoolContract(poolAddress) {
        ...
        if (!contract) {
            contract = await this.tronWeb.contract(pool_abi_json_1.default, poolAddress);
            if (contract) {
    ...
    async getMinterContract(minterAddress) {
        let contract = this.minterContracts.get(minterAddress);
        if (!contract) {
            contract = await this.tronWeb.contract(minter_abi_json_1.default, minterAddress);
            if (contract) {
    ...
    async isMethodSupportedByContract(contractAddress, methodName) {
