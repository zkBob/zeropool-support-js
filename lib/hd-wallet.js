    ...
    async enableCoin(coinType) {
        ...
        switch (coinType) {
            case coin_type_1.CoinType.near: {
                coin = new near_1.NearCoin(this.seed, this.config.near, this.worker);
                break;
            }
            case coin_type_1.CoinType.ethereum: {
                ...
                const backend = new relayer_1.RelayerBackend(new URL(this.config.ethereum.relayerUrl), web3, state, this.snarkParams, this.config.ethereum, this.worker);
                coin = new ethereum_1.EthereumCoin(this.seed, web3, this.config.ethereum, backend, this.worker);
                break;
            }
            case coin_type_1.CoinType.waves: {
                coin = new waves_1.WavesCoin(this.seed, this.config.waves, this.worker);
                break;
            }
            default: {
