    ...
    static async init(seed, config) {
        ...
        for (let networkType in network_type_1.NetworkType) {
            if (config[networkType]) {
                promises.push(wallet.enableNetwork(networkType, config[networkType]));
            }
        }
    ...
    async enableNetwork(networkType, config) {
        ...
        switch (networkType) {
            ...
            case network_type_1.NetworkType.aurora: {
                ...
                const state = await state_1.ZeroPoolState.create(sk, networkType, BigInt(1000000000));
                const web3 = new web3_1.default(config.httpProviderUrl);
                const tokens = config.networks[networkType];
                const backend = new relayer_1.RelayerBackend(tokens, web3, state, this.snarkParams, this.worker);
                network = new evm_1.EthereumNetwork(this.seed, web3, config, state, backend, this.worker);
                break;
            }
